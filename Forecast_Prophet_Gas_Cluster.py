...
#####################################################################################################################################
    ...
    if tipo_forecast == 'A' or tipo_forecast == 'P':
        ...
        cursorC.execute("""select distinct idCluster from DB_Gas.dbo.tbl_Codifica_Cluster_Gas where Mesi >= 12 and idCluster != 57""")
        ...
        for cl in range(len(ragsoc)):
            ...
            join DB_Gas.dbo.tbl_Anagrafica_Cluster_Gas a on g.pdr = a.pdr and idCluster = ?
            group by Data
            ...
            if len(df) > 0:
                ...
                x = {'Data':data, 'Forecast':consumo, 'idCluster':nome}
                ...
                cursorC.execute("""select Consumo from Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster where idCluster = ? and Data = ? and idAlgoritmo = 5""", int(result['idCluster'].values[0]), result['Data'].values[0])
                k = cursorC.fetchall()

                if len(k) == 0:
                    for j in range(len(result)):
                        cursorC.execute("""insert into Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster values (?, ?, ?, 5)""", result['Data'].values[j], int(result['idCluster'].values[0]) ,result['Forecast'].values[j])
                        connC.commit()
                else:
                    for j in range(len(result)):
                        cursorC.execute("""update Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster set Consumo = ? where idCluster = ? and idAlgoritmo = 5 and Data = ?""", result['Forecast'].values[j], int(result['idCluster'].values[0]), result['Data'].values[j])
                        connC.commit()
...
#####################################################################################################################################
    if tipo_forecast == 'A' or tipo_forecast == 'G':
        ...
        for cl in range(len(pdr)):
            ...
            cursorC.execute("""select Data, isnull(sum(Consumo), 0)
                from DB_Gas.dbo.tbl_Misure_Giornaliere_SBG g
                join DB_Gas.dbo.tbl_Anagrafica_Cluster_Gas a on g.pdr = a.pdr and a.idCluster = ?
                join DB_Gas.dbo.tbl_Codifica_Cluster_Gas c on c.idCluster = a.idCluster
                where g.Data >= dateadd(month, -c.Mesi, ?)
                group by Data
                order by Data""", pdr[cl], dd)
            t = cursorC.fetchall()
            ...
            # -- estraggo il periodo di stagionalità simile e imposto la previsione come ricalendarizzazione di quel periodo 
                ...
                if len(r) == 0:
                    ...
                    from DB_Gas.dbo.tbl_Misure_Giornaliere_SBG g
                    join Db_Gas.dbo.tbl_Anagrafica_Cluster_Gas a on a.pdr = g.pdr and a.idCluster = ?
                    join DB_Gas.dbo.tbl_Codifica_Cluster_Gas c on c.idCluster = a.idCluster
                    where DATEPART(month, data) = (select top 1 DATEPART(month, data) from DB_Gas.dbo.tbl_Misure_Giornaliere_SBG g
                    join Db_Gas.dbo.tbl_Anagrafica_Cluster_Gas a on a.pdr = g.pdr and a.idCluster = ? order by Data desc)
                    and g.Data >= dateadd(month, -c.Mesi, ?)
                    group by Data
                    order by Data""", pdr[cl], pdr[cl], dd)
                ...
                if len(r) == 0:
                    ...
                    join Db_Gas.dbo.tbl_Anagrafica_Cluster_Gas a on a.pdr = d.pdr and a.idCluster = ?
                    ...
                    where d.DataMese = ?
                    group by p.Data
                    order by p.Data""", pdr[cl], datainizio, datafine, dd)
            ...
            if len(data) != len(consumo) or len(data) != len(nome) or len(consumo) != len(nome):
                ...
                for j in range(len(result)):
                    cursorC.execute("""select Consumo from Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster where idCluster = ? and Data = ? and idAlgoritmo = 5""", int(result['pdr'].values[0]), result['Data'].values[j])
                    kk = cursorC.fetchall()

                    if len(kk) == 0:
                        cursorC.execute("""insert into Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster values (?, ?, ?, 5)""", result['Data'].values[j], int(result['pdr'].values[j]) ,result['Forecast'].values[j])
                        connC.commit()
                    else:
                        cursorC.execute("""update Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster set Consumo = ? where idCluster = ? and idAlgoritmo = 5 and Data = ?""", result['Forecast'].values[j], int(result['pdr'].values[j]), result['Data'].values[j])
                        connC.commit()
...
#####################################################################################################################################
    ...
    cursorC.execute("""delete from Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster
                   where idCluster = 57 and Data between ? and dateadd(day, -1, dateadd(month, 1, ?))""", dd, dd)

    cursorC.execute("""insert into Db_Gas.dbo.tbl_Forecast_Medio_Termine_Cluster
        select Data, 57, sum(Consumo), 5
        from DB_Gas.dbo.tbl_Forecast_Medio_Termine_Pdr
        where pdr = '-' and Data between ? and dateadd(day, -1, dateadd(month, 1, ?))
        group by Data
        order by Data""", dd, dd)
